...
the compiling command is (with a correct system configuration):
...
will result as in the first case.
...
- a question mark indicates a condition, the variable is checked
  ...
  if the result is True (see CONDITIONS), es:
...
- `"` a double quotation indicates constants,
  a constant may contain every character until the end of line,  
      ...
      @other:"text that contains # character, is not a comment
...
- `#` a hash character indicates that next string will be parsed 
  as a document row, it may contains all variables already defined,  
      ...
      @var:##user# is the owner of this document (#name>#).
...
- `?` a question mark indicates a requested interactive input,  
  ...
  **TODOs** the type of input may be checked, using regexp, where the last question mark 
...
- an exclamation mark indicates a value searched in system environment variables,
  then, if misses, in COMMAND LINE ARGMENTS and then is asked like with '?',
      ...
      !env-or-args-name!regexp!user question
...
Conditions need to be robust (however I want to implement that directly), 
so that have a generic syntax.
Evaluate a condition mean to compare a variable against an expression, 
that is:
    ...
    <less then>  : a comparison character '<'
                   True if the variable value is less then <expression> value
    <more then>  : a comparison character '>'
                   True if the variable value is more then <expression> value
...
+ if **True** execute next definition, 
+ if **False** it will be omitted and execution proceeds with second next.
...
- **Date** : date is rendered in ISO compressed format.
      ...
      1 April 2015 => 20150401
  
  this is easy to split in parts with text distribution 
  (as xxxx/xx/xx) or date separator
- **Time** : time is rendered in compressed precise format.
  
      13h 25' 14'' 123456 mcsec => 132514123456

  this is easy to render in human readable format with text distribution
  or time separators
...
- **text distribution** text is spread over the map where:  
  'x' stands for variable character,  
  other chars stands for them-self (except for '\_' that is
  the 'end-of-options' char).
...
- **dot** `'.'` pilots the render of separators, default is a dot
  itself, alternatives may be indicated immediately after dot.
...
- **shifting** the date may be anticipated and postponed, the
  simple way is using the number of days: tomorrow will be '+1',
  a week ago will be '-7':
  
      #date:-7#      ===>     20150414

  otherwise the explicit syntax is '[+-]n[dLmY]' and it is iterable:

      #date:-1m:+4d# ===>     20150325

  where 'd' means days, 'L' means working days, that are counted 
  jumping over saturday and sunday, 'm' means months and 'Y' 
  means years.
...
- **dot** `'.'` pilots the render of separators, default is a
  ...
  alternatives may be indicated immediately after dot, es:

      #date..___#    ===>     12.07.42.18

- **position** positioning of folds may be changed with characters
  'H', 'M', 'S' or 'f', meaning hours, minutes, seconds and 
  fractions of second.
...
Predefined variables are a facility for often-used data.
    ...
    name        contains the first parameter passed in command line, if 
                ...
                'name=...', if none is found it take the name of the template.
...
_Character needing escaping and mapping requested_
...
Other optional arguments are described in the program (use -h).
